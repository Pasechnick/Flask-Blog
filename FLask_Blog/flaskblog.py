...
from flask_sqlalchemy import SQLAlchemy # importing the database library
...
# doing a database as classes-model is the right way
# so the each class will be its own table in the database, so we make columns
...
class User(db.Model):
    ...
    password = db.Column(db.String(60), nullable=False) # those will be hashed (encoded)
